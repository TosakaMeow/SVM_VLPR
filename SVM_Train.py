...
class SVM(StatModel):
    def __init__(self, C=1, gamma=10):
...
def preprocess_hog(digits):
    samples = []
    for img in digits:
        gx = cv2.Sobel(img, cv2.CV_32F, dx=1, dy=0, ksize=3)
        gy = cv2.Sobel(img, cv2.CV_32F, dx=0, dy=1, ksize=3)
...
def train_svm(path):
    # 识别英文字母和数字
    Model = SVM(C=10, gamma=0.01)
    # 识别中文
    Modelchinese = SVM(C=10, gamma=0.01)
    # 英文字母和数字部分训练
    chars_train = []
    chars_label = []
